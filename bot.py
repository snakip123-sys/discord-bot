            ...
            if minutes >= 1:
                ...
                while data[user_id]["xp"] >= (data[user_id]["level"] + 1) * 100:
                    ...
                    if channel and channel.guild.system_channel:
                        await channel.guild.system_channel.send(f"{member.mention} повысился до {data[user_id]['level']} уровня!")
...
async def casino(ctx, bet: int = 15):
    ...
    if xp_gain > 0:
        ...
        tasks_data = load_tasks()
        today = datetime.now().strftime("%Y-%m-%d")
        if user_id_str not in tasks_data or tasks_data[user_id_str].get("date") != today:
        ...
        tasks_data[user_id_str]["casino_wins"] += 1
        save_tasks(tasks_data)
...
async def daily(ctx):
    ...
    tasks_data[user_id]["daily_done"] = True
    save_tasks(tasks_data)
...
async def top(ctx):
    ...
    for i, (user_id, stats) in enumerate(sorted_users, 1):
        ...
        embed.add_field(name=f"{i}. {user.name}", value=f"Уровень: {stats['level']} | Опыт: {stats['total_xp']}", inline=False)
...
async def tasks(ctx):
    ...
    embed.add_field(name="Получить ежедневный бонус", value="Выполнено: Да" if user_tasks['daily_done'] else "Выполнено: Нет", inline=False)
